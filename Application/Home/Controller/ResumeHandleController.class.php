...
class ResumeHandleController extends CompanyBaseController {
	...
	public function show()
	{
		...
		$res_hopejob=$hopejob->where($rid_map)->find();
		if($res_hopejob){
		...
		//工作经历
		$res_work_history=$work_history->where($rid_map)->select();
		if($res_work_history){
		...
		//项目经验
		$res_project=$project->where($rid_map)->select();
		if($res_project){
		...
		//教育背景
		$res_education=$education->where($rid_map)->select();
		if($res_education){
		...
		//作品展示
		$res_show_works=$show_works->where($rid_map)->select();
		if($res_show_works){
	...
	public function showAudition()
	{
		...
		$res = $sendObj->alias('AS s')
					   ...
					   ->join('LEFT JOIN `oj_resume` AS r ON s.user_id = r.id')
					   ->join('LEFT JOIN `oj_job` AS j ON s.job_id = j.id')
