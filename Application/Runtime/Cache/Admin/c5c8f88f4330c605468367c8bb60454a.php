<?php if (!defined('THINK_PATH')) exit();?><div class="modal-header no-padding">
	<div class="table-header">
		<i class="icon-lock"></i>
		<button type="button" class="close" data-dismiss="modal">&times;</button>
		修改用户【$info.username】【 <?php echo C('user_type')[$info['type']] ?>】资料
	</div>
</div>

<div class="modal-body overflow-visible">
	<div class="control-group">
		<label class="control-label" for="username">用户名</label>
		<div class="controls">
			<input type="text" required="required" disabled value="<?php echo ($info["username"]); ?>" id="username" placeholder="用户名" maxlength="10"/>
		</div>
	</div>

	<div class="control-group">
		<label class="control-label" for="gid">状态</label>
		<div class="controls">
			<select name="state">
			<?php foreach (C('user_state') as $key => $value): ?>
				<option value="<?php echo ($key); ?>" <?php if ($key == $info['state']): ?>
					selected="selected"
				<?php endif ?>><?php echo ($value); ?></option>				
			<?php endforeach ?>
			</select>
		</div>
	</div>
</div>

<div class="modal-footer">
	<input type="hidden" name="id" value="<?php echo ($info["id"]); ?>" id="hidden-userid"/>
	<button data-dismiss="modal" class="btn btn-small">
		<i class="icon-remove"></i>
		取消
	</button>
	<button type="button" class="btn btn-small btn-primary" onclick="$.action('<?php echo U('Admin/User/doEdit');?>', this);">
		<i class="icon-ok"></i>修改
	</button>
</div>